...
def create_expected_entries(regex_list):
    ...
    dict_name_block_part = OrderedDict()
    no_duplicate_main_list_block_parts = []  # OrderedDict maintains order of key:value insertion order
...
def regex(the_file):
    ...
    for i in the_file:
        ...
        # regex_actual_parts = '^\s*([0-9]+)\s+(T[0-9]+)\s+.*(/\S*/\S*\s*\S*\s*\S*/\S*/\S*/\S*/\S*/)$'
        ...
        # CONSISTS OF THE MISSING ENTRIES D FROM DATA
        for t in the_expected_entries:
            if t in actual_parts:
                # print t
    ...
    for a in master_data:
        print(a)

    # with open('missing.txt', mode="w") as missing_file_txt:
    #     for line in master_data:
    #         # print line
    #         missing_file_txt.write("%s\n" % line)
